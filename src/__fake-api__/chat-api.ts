...
const findThreadById = (threadId: any) => {
  const thread = threads.find((_threadId) => _threadId.id === threadId);

  return thread || null;
};

const findThreadByParticipantIds = (participantIds: any) => {
...
class ChatApi {
  ...
  addMessage({ threadId, recipientIds, body }: I) {
    return new Promise((resolve, reject) => {
      try {
