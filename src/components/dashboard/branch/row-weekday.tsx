...
import { Box, Switch, TableCell, TableRow, Typography } from "@mui/material";
import { DesktopTimePicker } from "@mui/x-date-pickers/DesktopTimePicker";
import React, { useEffect, useState } from "react";
// import { useSelector } from '../../../store';
type RowWeekdayProps = {
  item: any;
  index: any;
  formik: any;
  arrayHelpers: any;
};
const RowWeekday: React.FC<RowWeekdayProps> = (props) => {
  ...
  const setValidationError = (checked: boolean) => {
    if (checked) {
  ...
  const onChange = (val:any, field:any) => {
    if (val?.target) {
      arrayHelpers.replace(index, {
        ...
        openTime: val.target.checked
          ? null
          : formik.values.weekdaySchedules[index]?.openTime,
        endTime: val.target.checked
          ? null
  ...
  const handleMinTime = (val:any) => {
    setMinTime(val);
  };
  const handleMaxTime = (val:any) => {
    setMaxTime(val);
  };
  ...
  return (
    <TableRow key={`${item.weekday}`}>
      <TableCell sx={{ textTransform: "capitalize" }} width="15%">
        {item.weekday}
      </TableCell>
      <TableCell width="35%">
        <DesktopTimePicker
        ...
        //   fullWidth
          maxTime={maxTime}
          onChange={(val) => {
            onChange(val, "openTime");
            handleMinTime(val);
          }}
          label="Open Time"
          value={formik.values.weekdaySchedules[index]?.openTime}
      ...
      <TableCell width="35%">
        <DesktopTimePicker
          ...
          // value={}
          minTime={minTime}
          value={formik.values.weekdaySchedules[index]?.endTime}
        //   fullWidth
          label="End Time"
