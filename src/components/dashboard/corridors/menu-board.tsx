...
const MenuBoard = ({ branch }) => {
    const { menu } = useSelector((state) => state);

    const dispatch = useDispatch();
    useEffect(() => {
        if (branch)
            dispatch(getBoard({ branchId: branch?.id }))
    }, [branch])
    ...
    const handleDragEnd = async ({ source, destination, draggableId, type }) => {
        try {
            ...
            if (type === "grid") {
                ...
                const [destinationCorridor, destinationRow] = destination.droppableId.split("|");
                if (sourceCorridor === destinationCorridor && destinationRow === sourceRow && source.index === destination.index) {
                    return;
                }
                const length = 3;
                // reduce 1 row if the destination is greater than source
                let excp = (sourceRow < destinationRow) && (sourceCorridor !== destinationCorridor) ? -1 : 0;
                ...
                if (sourceCorridor === destinationCorridor) {
                    await dispatch(moveProductCard({ cardId: draggableId, position, branchId: branch?.id }));
                } else {
                    // // Moved to another column/row
                    await dispatch(moveProductCard({ cardId: draggableId, position, rowId: destinationCorridor, branchId: branch?.id }))
                }
            }
                ...
                <Box sx={{ mb: 4 }}>
                    ...
                    >
                        ...
                        <DragDropContext onDragEnd={handleDragEnd}>
                            ...
                            {menu.rows.allIds.length > 0 && menu?.rows.allIds.map((rowId) => (
                                <CorridorRow
                                    rowId={rowId}
                                    key={`row_${rowId}`}
                                />
